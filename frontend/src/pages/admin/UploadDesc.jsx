  ...
  return (
    <div className="w-full space-y-6 pb-12">
      <button onClick={() => navigate("/admin/knowledge-ai")} className="flex items-center gap-2 text-sm text-gray-400 transition hover:text-white">
        <ArrowLeft size={18} /> Kembali ke Knowledge AI
      </button>

      <div>
        <h1 className="text-2xl font-bold text-white sm:text-3xl">Tulis Deskripsi Manual</h1>
        <p className="mt-1 text-sm text-gray-400 sm:text-base">Masukkan masalah mesin, gejala, dan solusi teknis ke dalam Knowledge Base.</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="rounded-2xl border border-[#1F2937] bg-[#121620] p-5 sm:p-6">
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              <div className="md:col-span-2">
                <label className="mb-2 block text-sm font-medium text-gray-300">Judul Kasus</label>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-300">Tipe Mesin</label>
          ...
          <div className="rounded-2xl border border-[#1F2937] bg-[#121620] p-5 sm:p-6">
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-gray-300">Gejala (Symptoms)</label>
                ...
                <button type="button" onClick={() => addArrayItem(setSymptoms, symptoms)} className="mt-2 flex items-center gap-2 text-sm font-medium text-[#A855F7] hover:text-[#C084FC]"><Plus size={16} /> Tambah Gejala</button>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-[#1F2937] bg-[#121620] p-5 sm:p-6">
            ...
            <div>
              ...
              <button type="button" onClick={() => addArrayItem(setSolutions, solutions)} className="mt-2 flex items-center gap-2 text-sm font-medium text-[#10B981] hover:text-[#34D399]"><Plus size={16} /> Tambah Langkah</button>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3">
            <button type="submit" disabled={isSubmitting} className="flex items-center gap-2 rounded-xl bg-[#7C3AED] px-6 py-3 text-sm font-semibold text-white hover:bg-[#6D28D9] disabled:opacity-50">
              {isSubmitting ? <><Loader2 size={18} className="animate-spin" /> Menyimpan...</> : <><Save size={18} /> Simpan Knowledge</>}
            </button>
          </div>
        </form>
      )}
